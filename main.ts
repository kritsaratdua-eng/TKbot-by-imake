...
namespace TinkerBott {
    ...
    //% blockId=april_tag_set_mode block="set AI camera to AprilTag recognition mode"
    //% group="AprilTag"
    //% weight=50
    export function setAprilTagMode(): void {
        aiSetMode(AIMode.TagRecognition);
    }

    //% blockId=april_tag_detect block="is AprilTag %id detected?"
    //% group="AprilTag"
    //% weight=45
    export function isAprilTagDetected(id: number): boolean {
        aiRequestData();
        return aiIsDetected() && aiGetId() === id;
    }

    //% blockId=april_tag_get_position block="get AprilTag %id position (X,Y)"
    //% group="AprilTag"
    //% weight=40
    export function getAprilTagPosition(id: number): string {
        if (isAprilTagDetected(id)) {
            return aiGetX() + "," + aiGetY();
        }
        return "0,0"; // Not detected - return center position
    }

    //% blockId=april_tag_follow block="follow AprilTag %id at speed %speed"
    //% speed.min=0 speed.max=100
    //% group="AprilTag"
    //% weight=35
    export function followAprilTag(id: number, speed: number): void {
        if (!isAprilTagDetected(id)) {
            stopcar();
            return;
        }
    ...
    //% blockId=april_tag_get_distance block="get distance to AprilTag %id (estimated)"
    //% group="AprilTag"
    //% weight=30
    export function getAprilTagDistance(id: number): number {
        if (!isAprilTagDetected(id)) {
            return 0; // Not detected
        }

        let width = aiGetWidth();
        // Rough estimation: larger width means closer distance
        // This is a simple approximation - actual distance calculation would need camera calibration
        if (width > 100) return 10; // Close
        if (width > 50) return 20;  // Medium
        if (width > 20) return 30;  // Far
        return 50; // Very far
    }

    //% blockId=april_tag_stop_at block="stop when reaching AprilTag %id at distance %distance cm"
    //% group="AprilTag"
    //% weight=25
    export function stopAtAprilTag(id: number, distance: number): void {
        let currentDistance = getAprilTagDistance(id);
